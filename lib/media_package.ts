import { Aws, aws_iam as iam, Duration, CfnOutput, aws_mediapackage as mediapackage } from "aws-cdk-lib";
...
export class MediaPackageCdnAuth extends Construct {
  ...
  constructor(scope: Construct, id: string, configuration: any) {
    ...
    const dashEndpoint = new mediapackage.CfnOriginEndpoint(this, "DashEndpoint", {
      channelId: this.myChannel.id,
    ...
    const cmafEndpoint = new mediapackage.CfnOriginEndpoint(this, "cmafEndpoint", {
      channelId: this.myChannel.id,
      id: Aws.STACK_NAME + "-cmaf-" + this.myChannel.id,
      cmafPackage,
    });
